...
class OlmoeParallelModel(nn.Module):
    def __init__(self, config, pmap=None, memory_tracker=None):
        ...
        self.layers = nn.ModuleList(
            [OlmoeParallelDecoderLayer(config, layer_idx=layer_idx, pmap=pmap, memory_tracker=self.memory_tracker) for layer_idx in range(self.layer_start_idx, self.layer_end_idx)]
        )
...
class OlmoeParallelForCausalLM(nn.Module):
    ...
    def forward(self, input_ids, labels=None):
        hidden_states, router_logits = self.model(input_ids)
