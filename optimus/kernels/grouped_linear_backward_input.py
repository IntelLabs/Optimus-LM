...
def triton_grouped_linear_backward_input(output_grad, weights, input_grad, cum_token_counts):
...
def reference_grouped_linear_backward_input(output_grad, weights, input_grad, cum_token_counts):
...
if __name__ == "__main__":
    ...
    input_grad = torch.empty((num_tokens, ifm), dtype=dtype, device=device)
    ...
    for i in range(warmup_iters):
        output = triton_grouped_linear_backward_input(output_grad, weights, input_grad, cum_token_counts)
    ...
    # Reference path
    input_grad_ref = torch.empty((num_tokens, ifm), dtype=dtype, device=device)
    for i in range(warmup_iters):
        input_grad_ref = reference_grouped_linear_backward_input(output_grad, weights, input_grad_ref, cum_token_counts)
    torch.xpu.synchronize()
