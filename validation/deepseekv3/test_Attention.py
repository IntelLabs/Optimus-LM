...
from optimus.models.deepseek_v3.modeling_deepseek_v3 import DeepseekV3ParallelAttention
...
module_hf = DeepseekV3Attention(config_hf, layer_idx=layer_idx).to(dtype).to(device)
module = DeepseekV3ParallelAttention(config, layer_idx=layer_idx).to(dtype).to(device)
...
# Forward pass
output = module(input, position_embeddings)[0]
output_hf = module_hf(input_hf, position_embeddings_hf, attention_mask=None)[0]
...
# torch.Size([2, 128, 4, 64]) torch.Size([2, 1, 4, 64]) torch.Size([1, 4, 64]) torch.Size([1, 4, 64])
# torch.Size([2, 128, 4, 64]) torch.Size([2, 1, 4, 64]) torch.Size([1, 4, 56]) torch.Size([1, 4, 56])