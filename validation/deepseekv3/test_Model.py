...
from optimus.models.deepseek_v3.modeling_deepseek_v3 import DeepseekV3ParallelModel
...
output_grad = torch.randn((batch_size, sequence_length, config_hf.hidden_size), dtype=dtype, device=device)
...
module_hf = DeepseekV3Model(config_hf)
module = DeepseekV3ParallelModel(config)
...
# Forward pass
output = module(input)
output_hf = module_hf(input_hf).last_hidden_state
...
if not test_only_forward:
    print_error_info(module.embed_tokens.weight.grad, module_hf.embed_tokens.weight.grad, "Embedding weight gradients")