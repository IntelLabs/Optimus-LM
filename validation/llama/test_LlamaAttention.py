...
from optimus.models.llama.modeling_llama import LlamaParallelAttention, LlamaParallelRotaryEmbedding
...
# HF model
model_hf = LlamaAttention(config_hf, layer_idx=0)
# OM model
model = LlamaParallelAttention(config)
...
# Fwd/Bwd
output = model(hidden_states, position_embeddings)
output_hf = model_hf(hidden_states_hf, position_embeddings_hf, None)[0]
if not run_only_forward:
