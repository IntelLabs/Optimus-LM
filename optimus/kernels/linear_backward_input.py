...
@triton.jit
def triton_linear_backward_input_kernel(
    a_ptr, 
    b_ptr,
    c_ptr,
    M, N, K,
...
def triton_linear_backward_input(output_grad, weight, input_grad):
    M,K = output_grad.shape
    N = weight.shape[1]
    ...
    triton_linear_backward_input_kernel[grid](
        output_grad, 
        weight,
        input_grad, 
        M, N, K,
...
def reference_linear_backward_input(output_grad, weight, input_grad):
    torch.matmul(output_grad, weight, out=input_grad)
...
if __name__ == "__main__":
    ...
    input_grad = torch.empty((seq_length, ifm), dtype=dtype, device=device)
    
    # Warmup steps
    for i in range(warmup_iters):
        triton_linear_backward_input(output_grad, weight, input_grad)
    ...
    # Reference path
    input_grad_ref = torch.empty((seq_length, ifm), dtype=dtype, device=device)
    for i in range(warmup_iters):
        reference_linear_backward_input(output_grad, weight, input_grad_ref)
    torch.xpu.synchronize()
