...
# OLMo and OLMoE LLM Pretraining (Reference)
```
...
EXIT_STEPS=32 MODEL_CHOICE=allenai/OLMoE-1B-7B-0924 DATASET_CHOICE=allenai/OLMoE-mix-0924 USE_OM_SHARDED_OPTIMIZER=1 bash run_experiment.sh
```

# OLMoE* LLM Pretraining (Standard)
```
# Get four nodes (change -A appropriately)
...
python generate_serial_model.py --model_choice allenai/OLMoE-1B-7B-0924--SE1.125
...
# OLMoE* LLM Pretraining (Standard vs FastMoE)
```
# Get nodes
qsub -l select=8 -l walltime=01:00:00 -A Intel-Aurora  -q debug-scaling -I -l filesystems=flare
...
Serial model generation can be sped up by using "bash launch_dist.sh 12 <num_nodes> python generate_fast_serial_model.py --model_choice <model_choice>"

# DeepSeek-V3 Trillion parameter model pretraining
DeepSeek-V3--N264-L96 is a 1.1 Trillion parameter model with 58 Billion active parameters. It is obtained from the original DeepSeek-V3 by increasing the number of experts from 256 to 264 and the number of layers from 61 to 96. For dataset setup, refer to "Data preparation" section above. It is recommended to test the small debug model first to validate the setup.
...
bash launch_dist.sh 12 4 python generate_fast_dsv3_serial_model.py --model_choice deepseek-ai/DeepSeek-V3--L32-DL1-H2048-I10944-MI1408-N72-K6-SE2-A16
...
# Debug model (Run with DP2-PP4-EP12 parallelization config) (Add FORCE_UNIFORM_ROUTING=1 if it gets stuck)
...
bash launch_dist.sh 12 24 python generate_fast_dsv3_serial_model.py --model_choice deepseek-ai/DeepSeek-V3--N264-L96
