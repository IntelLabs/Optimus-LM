...
from transformers.models.olmoe.modeling_olmoe import OlmoeDecoderLayer, OlmoeRotaryEmbedding
# from optimus.models.olmoe.configuration_olmoe import OlmoeParallelConfig
# from optimus.models.olmoe.modeling_olmoe import OlmoeParallelDecoderLayer as OlmoeDecoderLayer
# from transformers.models.olmoe.modeling_olmoe import OlmoeRotaryEmbedding

# Optimus imports
from optimus.models.olmoe.configuration_olmoe import OlmoeParallelConfig
from optimus.models.olmoe.modeling_olmoe import OlmoeParallelDecoderLayer, OlmoeParallelRotaryEmbedding
...
with torch.no_grad():
    rotary_emb = OlmoeParallelRotaryEmbedding(config=config)
    position_ids = torch.arange(seq_length, device=device).unsqueeze(0)
    position_embeddings = rotary_emb(hidden_states, position_ids)

    rotary_emb_hf = OlmoeRotaryEmbedding(config=config_hf)
...
# HF model
model_hf = OlmoeDecoderLayer(config_hf, layer_idx=0)
# OM model
model = OlmoeParallelDecoderLayer(config, layer_idx=0, pmap=ParallelDpPpEpTpMapper())
...
# Fwd/Bwd
output = model(hidden_states, position_embeddings=position_embeddings)[0]
output_hf = model_hf(hidden_states_hf, position_embeddings=position_embeddings_hf)[0]
if not run_only_forward:
