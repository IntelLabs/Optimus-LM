...
def triton_grouped_linear_forward(input, weights, output, cum_token_counts):
    K = input.shape[1]
    N = weights[0].shape[0]
    ...
    weight_ptrs = torch.tensor(weight_addrs, device=input.device, dtype=torch.uint64)
    offsets = cum_token_counts.to(input.device)
...
def reference_grouped_linear_forward(input, weights, output, cum_token_counts):
    for g in range(len(weights)):
        start = cum_token_counts[g].item()
        end = cum_token_counts[g + 1].item()
        a_mat = input[start:end, :]
        b_mat = weights[g].t()
        c_mat = output[start:end, :]
        torch.matmul(a_mat, b_mat, out=c_mat)
    return output
...
if __name__ == "__main__":
    ...
    output = torch.empty((num_tokens, ofm), dtype=dtype, device=device)
    ...
    for i in range(warmup_iters):
        output = triton_grouped_linear_forward(input, weights, output, cum_token_counts)
    ...
    # Reference path
    output_ref = torch.empty((num_tokens, ofm), dtype=dtype, device=device)
    for i in range(warmup_iters):
        output_ref = reference_grouped_linear_forward(input, weights, output_ref, cum_token_counts)
    torch.xpu.synchronize()
