...
@triton.jit
def triton_linear_backward_input_add_kernel(
    a_ptr, 
    b_ptr,
    c_ptr,
    M, N, K, output_acc,
...
def triton_linear_backward_input_add(output_grad, weight, input_grad):
    ...
    triton_linear_backward_input_add_kernel[grid](
        output_grad, 
        weight,
        input_grad, 
        M, N, K, True,
...
def reference_linear_backward_input(output_grad, weight, input_grad):
    torch.addmm(input_grad, output_grad, weight, out=input_grad)
...
if __name__ == "__main__":
    ...
    input_grad = torch.randn((seq_length, ifm), dtype=dtype, device=device)
    input_grad_orig = input_grad.clone()

    # Warmup steps
    for i in range(warmup_iters):
        triton_linear_backward_input_add(output_grad, weight, input_grad)
    ...
    # Reference path
    input_grad_ref = input_grad_orig.clone()
    for i in range(warmup_iters):
